...
class Beds24Service {
   ...
   */
  async getProperties(propId?: string, includeAllRooms: boolean = true): Promise<{
    success?: boolean;
    count?: number;
    data?: Array<{
      id?: string | number;
  ...
  async getApartmentConfigurations(): Promise<Record<string, {
    ...
    try {
      ...
      if (propertiesData && typeof propertiesData === 'object' && 'data' in propertiesData && propertiesData.data) {
                  ...
                  if (apartmentKey) {
                    configurations[apartmentKey] = {
                      ...
                      bookingRules: {
                        ...
                        bookingType: String(prop.bookingRules?.bookingType || 'autoConfirmed'),
                        vatRatePercentage: Number(prop.bookingRules?.vatRatePercentage || 0)
                      }
                    };
                  }
                }
  ...
  private _parseOffersResponse(_data: unknown, _request: AvailabilityRequest): AvailabilityResponse {
    const available: string[] = [];
    const booked: string[] = [];
    const prices: Record<string, number> = {};

      console.log('Parsing offers response:', { data: _data, request: _request });

    // Generate date range first
    const startDate = new Date(_request.startDate);
    const endDate = new Date(_request.endDate);
    
    // Handle offers response format
    let offersData: unknown[] = [];
    
    if (_data && typeof _data === 'object') {
      if ('data' in _data && Array.isArray((_data as { data: unknown[] }).data)) {
        offersData = (_data as { data: unknown[] }).data;
   ...
   */
  // @ts-expect-error - Reserved for future use
  private _parseInventoryAvailabilityResponseV2(_data: unknown, _request: AvailabilityRequest): AvailabilityResponse {
    const available: string[] = [];
    const booked: string[] = [];
    const prices: Record<string, number> = {};

    console.log('Parsing availability response:', { data: _data, request: _request });
    ...
    if (_data && typeof _data === 'object') {
      if ('data' in _data && Array.isArray((_data as { data: unknown[] }).data)) {
        availabilityData = (_data as { data: unknown[] }).data;
      } else if (Array.isArray(_data)) {
        availabilityData = _data;
      }
    }
    ...
    const targetRoomId = _request.roomId ? parseInt(_request.roomId) : null;
    ...
    if (!roomData) {
      console.warn(`Room ${_request.roomId} not found in availability data`);
      // Return all dates as booked if room not found
      for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
        const dateStr = d.toISOString().split('T')[0];
        if (dateStr) {
          booked.push(dateStr);
          prices[dateStr] = this.getCalendarPrice(_request.roomId || '');
        }
      }
  ...
  private async parseInventoryOffersResponseV2(data: unknown, request: AvailabilityRequest & { adults: number; children?: number }, isCalendar?: boolean): Promise<AvailabilityResponse> {
    ...
    allDates.forEach(dateStr => {
      ...
      if (beds24Price && beds24Price > 0 && dateStr) {
    ...
    calendarData.forEach((item: unknown, index: number) => {
      if (item && typeof item === 'object') {
        ...
        if ('from' in itemObj && 'to' in itemObj && 
          ...
          dates = dateRange.map((date: Date) => format(date, 'yyyy-MM-dd')).filter((date): date is string => !!date);
        ...
        dates.forEach(date => {
          ...
          if (date && (date === '2025-12-31' || date === '2025-10-31' || date === '2025-11-30')) {
        ...
        if (index < 5) { // Log first few items for debugging
          console.log(`📅 Calendar item ${index + 1}:`, {
            dateRange: dates.length > 1 && dates[0] && dates[dates.length-1] ? `${dates[0]} to ${dates[dates.length-1]}` : dates[0] || 'unknown',
   ...
   */
  // @ts-expect-error - Reserved for future use
  private _parseInventoryResponseV2(_data: unknown, _request: AvailabilityRequest): AvailabilityResponse {
    const available: string[] = [];
    const booked: string[] = [];
    const prices: Record<string, number> = {};

    console.log('Parsing inventory response:', { data: _data, request: _request });
    ...
    if (_data && typeof _data === 'object') {
      ...
      if ('data' in _data && Array.isArray((_data as { data: unknown[] }).data)) {
        inventoryData = (_data as { data: unknown[] }).data;
      } else if ('inventory' in _data && Array.isArray((_data as { inventory: unknown[] }).inventory)) {
        inventoryData = (_data as { inventory: unknown[] }).inventory;
      } else if (Array.isArray(_data)) {
        inventoryData = _data;
      }
    }
      ...
      if (item && typeof item === 'object') {
        ...
        if (date) {
          inventoryMap[date] = {
            ...
            price: price || this.getCalendarPrice(_request.roomId || '')
          };
        }
      }
    });
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      ...
      if (dateStr) {
        ...
        if (inventory && inventory.available > 0) {
          ...
          booked.push(dateStr);
          prices[dateStr] = inventory?.price || this.getCalendarPrice(_request.roomId || '');
        }
      }
    }
   ...
   */
  // @ts-expect-error - Reserved for future use
  private _parseAvailabilityResponseV2(_data: unknown, _request: AvailabilityRequest): AvailabilityResponse {
    const available: string[] = [];
    const booked: string[] = [];
    const prices: Record<string, number> = {};
    ...
    if (_data && typeof _data === 'object') {
      ...
      if ('data' in _data && Array.isArray((_data as { data: unknown[] }).data)) {
        bookings = (_data as { data: unknown[] }).data;
      } else if (Array.isArray(_data)) {
        bookings = _data;
      }
    }
    ...
    for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
      ...
      if (dateStr) {
        ...
        const guestCount = _request.numAdults || 2;
        let basePrice = this.getCalendarPrice(_request.roomId || '');
          ...
          const finalPrice = this.applyDiscounts(basePrice, _request.startDate, _request.endDate);
          prices[dateStr] = finalPrice;
        }
      }
    }
   ...
   */
  // @ts-expect-error - Reserved for future use
  private _getDefaultPrice(_roomId?: string, _startDate?: string, _endDate?: string, _guestCount?: number): number {
    console.log(`🔍 getDefaultPrice called with: roomId=${_roomId}, guestCount=${_guestCount}, startDate=${_startDate}, endDate=${_endDate}`);
    
    // Get base price
    let basePrice = this.getCalendarPrice(_roomId || '');
    
    // Apply guest count pricing (MORE GUESTS = HIGHER PRICE)
    if (_guestCount && _guestCount > 2) {
      const guestMultiplier = 1 + ((_guestCount - 2) * 0.15); // +15% per extra guest above 2
      basePrice = basePrice * guestMultiplier;
      console.log(`👥 Guest count pricing: ${_guestCount} guests → ${Math.round(basePrice * 100) / 100}€/night`);
    }
    
    // Apply unified discount logic
    const finalPrice = this.applyDiscounts(basePrice, _startDate, _endDate);
    console.log(`💰 Final price for roomId ${_roomId}: ${finalPrice}€/night`);
    return finalPrice;
  }
