...
export function AvailabilityCalendar({
  ...
  const { data: availability, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const result = await getApartmentAvailability(apartmentSlug, monthStart, monthEnd);
      
      // Debug logging for calendar availability
      console.log(`🗓️ Calendar availability for ${apartmentSlug}:`, {
        monthStart: monthStart.toISOString().split('T')[0],
        monthEnd: monthEnd.toISOString().split('T')[0],
        availableDays: result.available?.length || 0,
        bookedDays: result.booked?.length || 0,
        bookedDates: result.booked || [],
        sampleAvailable: result.available?.slice(0, 5) || []
      });
      
      return result;
    },
    staleTime: 0, // Always fresh data for accurate bookings
    refetchOnWindowFocus: true,
    refetchInterval: 30 * 1000 // Refresh every 30 seconds for real-time bookings
  });
